<?php
    if(isset($_POST['id']) && !empty($_POST['id'])) {
        ...
        foreach($stmt_get_sizes as $row_sizes)
        {
            ...
            echo "<input required class='input-size col-4' name='size[]' type='text' value='".$row_sizes['size']."'></input>";
            $size = $row_sizes['size'];
            echo "<input required class='input-size col-4' name='amount[]' type='number' value='".$row_sizes['amount']."'>";
        ...
        <td><button type='submit' name='delete-size-send' class='delete_record col-4' value="<?=$primary_id?>">USUŃ</button></td>
        </form>  
        </div>    
            <?php
        }
